...
const Slider = () => {
	...
	const [imagesArray, setImagesArray] = useState(myArr);
	...
	const handleDeleteImage = (index) => {
		let currentArr = [...imagesArray];
		delete currentArr[index];
		console.log(currentArr);
		setImagesArray(currentArr.filter(Boolean));
	};

	return (
		<div>
			<div class="big-img">
				<img src={imagesArray[currentImage]} />
			</div>
			<div class="images">
				{imagesArray.map((imgLink, index) => (
					<img
						key={imgLink}
						onMouseEnter={() => handleSetImage(index)}
						onClick={() => handleDeleteImage(index)}
						style={{
							width: 100,
							border: currentImage === index ? "solid 3px red" : null,
